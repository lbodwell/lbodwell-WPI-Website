...
let FTC = (function() {
	...
	let enemies = [
		...
		[[16, 21, null, -1], [27, 4, null, -1]],
		[[6, 15, null, -1]],
		[[4, 3, null, -1], [25, 15, null, -1]]
	];
	let vaccinePieces = [
		[[19, 11], [30, 15]],
		[[28, 4], [29, 27]],
	...
	function playerStep(h, v) {
		...
		switch (level) {
			...
			case 3: {
				...
				// connects to 2 (middle left) and 4 (bottom left)
			}
			break;
			case 4: {
				if ((ny === 11 || ny === 12 || ny === 13) && nx < 0) {
					level = 5;
					loadMap();
					playerPlace(31, ny);
				} else if ((ny === 30 || ny === 31) && nx > 31) {
					level = 3;
					loadMap();
					playerPlace(0, ny);
				}
				// connects to 5 (top left) and 3 (bottom right)
			}
				break;
			case 5: {
				if (ny > 31 && (nx === 26 || nx === 27 || nx === 28)) {
					level = 6;
					loadMap();
					playerPlace(nx, 0);
				} else if ((ny === 11 || ny === 12 || ny === 13) && nx > 31) {
					level = 4;
					loadMap();
					playerPlace(0, ny);
				}
				// connects to 6 (bottom right) and 4 (middle right)
			}
				break;
			case 6: {
				if (ny < 0  && (nx === 26 || nx === 27 || nx === 28)) {
					level = 5;
					loadMap();
					playerPlace(nx, 31);
				}
				// connects to 5 (top right)
			}
		}
		...
		updateStatus();
		if (numVaccinePieces === TOTAL_VACCINE_PIECES) {
			isWinner = true;
		}
		playerPlace(nx, ny);
	}
